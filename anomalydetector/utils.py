...
def make_cheb_basis(xs, num_basis, domain):
    ...
    for _ix in range(num_basis):
        basis_generator = chebyshev.basis(_ix, window=(-1, 1), domain=domain)
...
def reshape_for_cheb(x_mat, y_mat, num_basis):
    ...
    x_flat = (c_reshaped * x_mat_reshaped).reshape(
        ndays * nvals_per_day, nfeatures * num_basis
    )
    y_flat = y_mat.ravel()
...
def optimal_weighted_regression(
    X, y, weights, lambda_values, n_splits=5, random_state=0
):
    """
