...
class MultiDataHandler:
    def __init__(
        ...
        dict_handler = {
            key: DataHandler(dict_dataframes[key]) for key in dict_dataframes
        }
    ...
    def align(self):
        ...
        if self.good_days is None:
            ...
            self.common_days = self.raw_handler[self.keys[0]].day_index[
                self.good_days[self.keys[0]]
            ]
        if self.filled_mat is None:
            dict_mat = {}
            for key in tqdm(self.keys, desc="Aligning datasets"):
                dict_mat[key] = self.raw_handler[key].filled_data_matrix[
                    :, self.good_days[key]
                ]
    ...
    def dilate(self, ndil=None):
        ...
        if ndil is None:
            ...
            for key in self.raw_handler:
                dil = Dilation(
                    self.raw_handler[key], **{"nvals_dil": ndil, "matrix": "filled"}
                )
        ...
        if (
            self.failure_mat is not None
            and self.target is not None
            and self.random_mat is not None
        ):
    ...
    def generate_failure(
        self, target, loss_distribution=None, proportion_totalday=0, seed=42
    ):
        """
        ...
        full_day = np.random.choice(
            [False, True], size=N, p=[1 - proportion_totalday, proportion_totalday]
        )
    ...
    def display(self, idx, site=None, ax=None):
        ...
        if site is None or site == self.target:
            ...
            ax.plot(
                x, self.failure_mat[:, idx], label="Power with outage", color="black"
            )
            ax.plot(
                x,
                self.dil_mat[self.target][:, idx],
                label="Power no outage",
                color="black",
                linestyle="dotted",
                linewidth=1,
            )
        ...
        ax.set_title(
            f"Display for day {self.common_days[idx].date()} ({site if site else self.target})"
        )
...
def train_test_split(
    multidata: MultiDataHandler, test_size=None, train_size=None, shuffle=True
):
