...
class TestOutagePipeline(unittest.TestCase):
    ...
    def test_fit_quantiles(self):
        ...
        data_file_path = (
            filepath / "fixtures" / "anomaly_detection" / f"one_month_{self.target}.csv"
        )
        with open(data_file_path) as file:
            data = np.loadtxt(file, delimiter=",")

        common_file_path = (
            filepath / "fixtures" / "anomaly_detection" / "common_day.csv"
        )
    ...
    def test_fit_linear(self):
        ...
        data_expected_file_path = (
            filepath / "fixtures" / "anomaly_detection" / "linear_coef.csv"
        )
        ...
        data_file_path = (
            ...
            / "fixtures"
            / "anomaly_detection"
            / "one_month_transform_failure.csv"
        )
        with open(data_file_path) as file:
